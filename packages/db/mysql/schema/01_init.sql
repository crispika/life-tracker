...
CREATE TABLE IF NOT EXISTS UC_GOAL (
  ...
  prefix_id INT(8) UNSIGNED NULL COMMENT '代码前缀ID',
  created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
...
CREATE TABLE IF NOT EXISTS UC_PROJECT (
  ...
  code VARCHAR(30) NOT NULL COMMENT '项目编号（如HT-1）',
  user_id INT(8) UNSIGNED NOT NULL COMMENT '所属用户ID',
  ...
  CONSTRAINT uk_uc_project_code UNIQUE (user_id, code)
...
-- 项目初始化状态设置表
CREATE TABLE IF NOT EXISTS UC_PROJECT_INIT_STATE (
  init_state_id INT(8) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  user_id INT(8) UNSIGNED NOT NULL,
  state_id INT(8) UNSIGNED NOT NULL COMMENT '新建项目时的默认状态ID',
  created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  
  CONSTRAINT fk_uc_project_init_state_user FOREIGN KEY (user_id) REFERENCES USER(user_id) ON DELETE CASCADE,
  CONSTRAINT fk_uc_project_init_state_state FOREIGN KEY (state_id) REFERENCES UC_PROJECT_STATE(state_id) ON DELETE RESTRICT,
  CONSTRAINT uk_uc_project_init_state_user UNIQUE (user_id)
) AUTO_INCREMENT=100000 COMMENT='用户项目初始化状态设置表';
