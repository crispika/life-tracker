...
const addTaskWorklog = async (
  ...
  try {
    ...
    await prisma.uC_TASK_WORKLOG.create({
      data: {
        task_id: taskId,
        user_id: userId,
    ...
    console.error('添加任务工作记录失败:', error);
    throw new Error(
      '添加任务工作记录失败: ' +
        (error instanceof Error ? error.message : String(error))
    );
...
const updateTaskWorklog = async (
  ...
  const worklog = await prisma.uC_TASK_WORKLOG.findUnique({
  ...
  try {
    await prisma.uC_TASK_WORKLOG.update({
      where: { worklog_id: logId, user_id: userId },
      data: {
        time_spent_minutes: timeSpent,
        note: note,
    ...
    console.error('更新任务工作记录失败:', error);
    throw new Error(
      '更新任务工作记录失败: ' +
        (error instanceof Error ? error.message : String(error))
    );
